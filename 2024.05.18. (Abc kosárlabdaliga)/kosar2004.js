...
function AdatKeszites(feldolgozandoLista) {
    var eredmenyekObjectLista = [];
    for (var _i = 0, feldolgozandoLista_1 = feldolgozandoLista; _i < feldolgozandoLista_1.length; _i++) {
        var elem = feldolgozandoLista_1[_i];
        var spliteltElem = elem.split(";");
        var elemObject = {
            hazai: spliteltElem[0],
            idegen: spliteltElem[1],
            hazai_pont: Number(spliteltElem[2]),
            idegen_pont: Number(spliteltElem[3]),
            helyszín: spliteltElem[4],
            időpont: spliteltElem[5]
        };
        eredmenyekObjectLista.push(elemObject);
    }
    return eredmenyekObjectLista;
}
var eredmenyekAdat = AdatKeszites(eredmenyek);
// 3. feladat
function RealMadridMerkozesekSzama(feldolgozandoLista) {
    var realMadridHazai = 0;
    var realMadridIdegen = 0;
    for (var _i = 0, feldolgozandoLista_2 = feldolgozandoLista; _i < feldolgozandoLista_2.length; _i++) {
        var elem = feldolgozandoLista_2[_i];
        if (elem.hazai == "Real Madrid") {
            realMadridHazai++;
        }
        else if (elem.idegen == "Real Madrid") {
            realMadridIdegen++;
        }
    }
    return [realMadridHazai, realMadridIdegen];
}
function RealMadridMerkozesekSzamaKiir(feldolgozandoLista) {
    console.log("3. feladat:");
    console.log("A Real Madrid: Hazai: ".concat(feldolgozandoLista[0], ", Idegen: ").concat(feldolgozandoLista[1]));
}
RealMadridMerkozesekSzamaKiir(RealMadridMerkozesekSzama(eredmenyekAdat));
//4. feladat
function DontetlenMerkozes(feldolgozandoLista) {
    var voltDontetlenMerkozes = false;
    for (var _i = 0, feldolgozandoLista_3 = feldolgozandoLista; _i < feldolgozandoLista_3.length; _i++) {
        var elem = feldolgozandoLista_3[_i];
        if (elem.hazai_pont == elem.idegen_pont) {
            voltDontetlenMerkozes = true;
        }
    }
    if (voltDontetlenMerkozes) {
        return "igen";
    }
    else {
        return "nem";
...
function DontetlenMerkozesKiir(feldolgozandoAdat) {
    console.log("4. feladat:");
    console.log("Volt d\u00F6ntetlen? ".concat(feldolgozandoAdat));
}
DontetlenMerkozesKiir(DontetlenMerkozes(eredmenyekAdat));
//5. feladat
function BarcelonaCsapatTeljesNev(feldolgozandoLista) {
    var teljesNev = "";
    for (var _i = 0, feldolgozandoLista_4 = feldolgozandoLista; _i < feldolgozandoLista_4.length; _i++) {
        var elem = feldolgozandoLista_4[_i];
        var spliteltElem = elem.hazai.split(" ");
        for (var _a = 0, spliteltElem_1 = spliteltElem; _a < spliteltElem_1.length; _a++) {
            var nev = spliteltElem_1[_a];
            if (nev == "Barcelona") {
                teljesNev = elem.hazai;
            }
        }
    }
    return teljesNev;
}
function BarcelonaCsapatTeljesNevKiir(feldolgozandoAdat) {
    console.log("5. feladat:");
    console.log("barcelonai csapat neve: ".concat(feldolgozandoAdat));
}
BarcelonaCsapatTeljesNevKiir(BarcelonaCsapatTeljesNev(eredmenyekAdat));
//6. feladat
function Merkozesek(feldolgozandoLista) {
    var merkozesek = [];
    for (var _i = 0, feldolgozandoLista_5 = feldolgozandoLista; _i < feldolgozandoLista_5.length; _i++) {
        var elem = feldolgozandoLista_5[_i];
        if (elem.időpont == "2004-11-21") {
            merkozesek.push("".concat(elem.hazai, "-").concat(elem.idegen, " (").concat(elem.hazai_pont, ":").concat(elem.idegen_pont, ")"));
        }
    }
    return merkozesek;
}
function MerkozesekKiir(feldolgozandoLista) {
    console.log("6. feladat:");
    for (var _i = 0, feldolgozandoLista_6 = feldolgozandoLista; _i < feldolgozandoLista_6.length; _i++) {
        var elem = feldolgozandoLista_6[_i];
        console.log(elem);
    }
}
MerkozesekKiir(Merkozesek(eredmenyekAdat));
//7. feladat
function Stadionok(feldolgozandoAdat) {
    var stadionok = [];
    for (var _i = 0, feldolgozandoAdat_1 = feldolgozandoAdat; _i < feldolgozandoAdat_1.length; _i++) {
        var elem = feldolgozandoAdat_1[_i];
        var szerepelE = false;
        for (var _a = 0, stadionok_1 = stadionok; _a < stadionok_1.length; _a++) {
            var stadion = stadionok_1[_a];
            if (elem.helyszín == stadion) {
                szerepelE = true;
            }
        }
        if (!szerepelE) {
            stadionok.push(elem.helyszín);
        }
    }
    return stadionok;
}
function MerkozesekSzama(feldolgozandoLista) {
    var merkozesekSzama = [];
    var stadionok = Stadionok(eredmenyekAdat);
    for (var _i = 0, stadionok_2 = stadionok; _i < stadionok_2.length; _i++) {
        var stadion = stadionok_2[_i];
        var merkozesek = 0;
        for (var _a = 0, feldolgozandoLista_7 = feldolgozandoLista; _a < feldolgozandoLista_7.length; _a++) {
            var elem = feldolgozandoLista_7[_a];
            if (stadion == elem.helyszín) {
                merkozesek++;
            }
        }
        merkozesekSzama.push(merkozesek);
    }
    return [stadionok, merkozesekSzama];
}
function HuszMerkozesFelettiStadinonok(feldolgozandoAdat) {
    var huszMerkozesFelettiStadinonok = [];
    var huszMerkozesFelettiStadinonokIndexLista = [];
    for (var i = 0; i < feldolgozandoAdat[1].length; i++) {
        if (feldolgozandoAdat[1][i] > 20) {
            huszMerkozesFelettiStadinonokIndexLista.push(i);
        }
    }
    for (var _i = 0, huszMerkozesFelettiStadinonokIndexLista_1 = huszMerkozesFelettiStadinonokIndexLista; _i < huszMerkozesFelettiStadinonokIndexLista_1.length; _i++) {
        var index = huszMerkozesFelettiStadinonokIndexLista_1[_i];
        huszMerkozesFelettiStadinonok.push("".concat(feldolgozandoAdat[0][index], ": ").concat(feldolgozandoAdat[1][index]));
    }
    return huszMerkozesFelettiStadinonok;
}
function HuszMerkozesFelettiStadinonokKiir(feldolgozandoLista) {
    console.log("7. feladat:");
    for (var _i = 0, feldolgozandoLista_8 = feldolgozandoLista; _i < feldolgozandoLista_8.length; _i++) {
        var elem = feldolgozandoLista_8[_i];
        console.log(elem);
    }
}
HuszMerkozesFelettiStadinonokKiir(HuszMerkozesFelettiStadinonok(MerkozesekSzama(eredmenyekAdat)));
