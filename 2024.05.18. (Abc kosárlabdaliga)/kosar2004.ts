...
function AdatKeszites(feldolgozandoLista: string[]): AdatInterface[] {
    let eredmenyekObjectLista: AdatInterface[] = [];

    for (const elem of feldolgozandoLista) {
        let spliteltElem: string[] = elem.split(";");
        let elemObject: AdatInterface = {
...
let eredmenyekAdat: AdatInterface[] = AdatKeszites(eredmenyek);


// 3. feladat

function RealMadridMerkozesekSzama(feldolgozandoLista: AdatInterface[]): [number, number] {
    let realMadridHazai: number = 0;
    let realMadridIdegen: number = 0;
...
function RealMadridMerkozesekSzamaKiir(feldolgozandoLista: number[]): void {
    console.log("3. feladat:")
    console.log(`A Real Madrid: Hazai: ${feldolgozandoLista[0]}, Idegen: ${feldolgozandoLista[1]}`);
}
...
function DontetlenMerkozes(feldolgozandoLista: AdatInterface[]): string {
    let voltDontetlenMerkozes: boolean = false;
...
function DontetlenMerkozesKiir(feldolgozandoAdat: string): void {
    console.log("4. feladat:");
    console.log(`Volt döntetlen? ${feldolgozandoAdat}`);
}
...
function BarcelonaCsapatTeljesNev(feldolgozandoLista: AdatInterface[]): string {
    let teljesNev: string = "";

    for (const elem of feldolgozandoLista) {
        let spliteltElem: string[] = elem.hazai.split(" ");


        for (const nev of spliteltElem) {
            if (nev == "Barcelona") {
...
function BarcelonaCsapatTeljesNevKiir(feldolgozandoAdat: string) {
    console.log("5. feladat:");
    console.log(`barcelonai csapat neve: ${feldolgozandoAdat}`);
}
...
function Merkozesek(feldolgozandoLista: AdatInterface[]): string[] {
    let merkozesek: string[] = [];

    for (const elem of feldolgozandoLista) {
        if (elem.időpont == "2004-11-21") {
            merkozesek.push(`${elem.hazai}-${elem.idegen} (${elem.hazai_pont}:${elem.idegen_pont})`);
        }
    }
...
function MerkozesekKiir(feldolgozandoLista: string[]): void {
...
function Stadionok(feldolgozandoAdat: AdatInterface[]): string[] {
    let stadionok: string[] = [];

    for (const elem of feldolgozandoAdat) {
        let szerepelE = false;

        for (const stadion of stadionok) {
...
function MerkozesekSzama(feldolgozandoLista: AdatInterface[]): [string[], number[]] {
    let merkozesekSzama: number[] = [];
    let stadionok: string[] = Stadionok(eredmenyekAdat);

    for (const stadion of stadionok) {
        let merkozesek: number = 0;
        for (const elem of feldolgozandoLista) {
...
function HuszMerkozesFelettiStadinonok(feldolgozandoAdat: [string[], number[]]): string[] {

    let huszMerkozesFelettiStadinonok: string[] = [];
    let huszMerkozesFelettiStadinonokIndexLista: number[] = [];

    for (let i: number = 0; i < feldolgozandoAdat[1].length; i++) {
        if (feldolgozandoAdat[1][i] > 20) {
            huszMerkozesFelettiStadinonokIndexLista.push(i);
        }
    }

    for (const index of huszMerkozesFelettiStadinonokIndexLista) {
        huszMerkozesFelettiStadinonok.push(`${feldolgozandoAdat[0][index]}: ${feldolgozandoAdat[1][index]}`);
    }

    return huszMerkozesFelettiStadinonok;
}

function HuszMerkozesFelettiStadinonokKiir(feldolgozandoLista: string[]): void {
