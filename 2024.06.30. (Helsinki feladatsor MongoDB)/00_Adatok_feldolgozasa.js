...
function HelsinkiAdatokFeldolgozasa(adatLista) {
    let ujHelsinkiLista = [];

    for (const item of adatLista) {
        const spliteltItem = item.split(" ");

        ujHelsinkiLista.push({
            Helyezes: Number(spliteltItem[0]),
            CsapatMeret: Number(spliteltItem[1]),
            Sportag: spliteltItem[2],
            VersenySzam: spliteltItem[3]
        })
    }

    return ujHelsinkiLista;
}

module.exports = HelsinkiAdatokFeldolgozasa(eredmenyek);